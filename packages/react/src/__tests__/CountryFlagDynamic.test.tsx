...
import { CountryFlagRounded } from '../CountryFlagRounded.js'
import { CountryFlagCircle } from '../CountryFlagCircle.js'
import type { CountryFlagProps } from '../types.js'

type DynamicComp = (props: CountryFlagProps) => ReturnType<typeof CountryFlagRounded>

const cases: { name: string; Comp: DynamicComp }[] = [
...
describe.each(cases)('$name', ({ Comp }) => {
  ...
  it('returns null for unknown codes and warns in development', () => {
  ...
  it('forwards ref on the title path too', () => {
    const ref = createRef<SVGSVGElement>()
    render(<Comp code="us" title="United States" ref={ref} />)
    expect(ref.current).not.toBeNull()
    expect(ref.current!.tagName.toLowerCase()).toBe('svg')
  })

  it('forwards arbitrary svg props', () => {
    const { container } = render(<Comp code="us" data-testid="flag" className="my-flag" />)
    const svg = container.querySelector('svg')!
